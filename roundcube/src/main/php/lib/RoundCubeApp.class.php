...
class OC_RoundCube_App {
  ...
  public static function refresh($rcHost, $rcPort, $maildir)
  {
    ...
    if ($rcl -> isLoggedIn()) {
      OCP\Util::writeLog('roundcube', 'Successfully refreshed the RC session.', OCP\Util::INFO);
    } else {
      OCP\Util::writeLog('roundcube', 'Failed to refresh the RC session.', OCP\Util::ERROR);
    }
  }
  ...
  public static function showMailFrame($rcHost, $rcPort, $maildir, $ownUser = false, $ownPass = false) {
    ...
    try {
      if (!$rcl->isLoggedIn()) {
        // If the login fails, display an error message in the loggs
        OCP\Util::writeLog('roundcube', 'Not logged in.', OCP\Util::ERROR);
        OCP\Util::writeLog('roundcube', 'Trying to refresh session.', OCP\Util::INFO);
