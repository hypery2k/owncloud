...
rcube_render_mailboxlist).addEventListener("responseafterrefresh",rcube_render_mailboxlist).addEventListener("afterimport-messages",function(){rcmail_ui.show_popup("uploadform",!1)})),"compose"==rcmail.env.action?rcmail_ui.init_compose_form():"show"!=rcmail.env.action&&"preview"!=rcmail.env.action||$('#attachment-list > li[id^="attach"]').each(function(){$(this).append($('<a class="drop"></a>').bind("click keypress",function(a){if("keypress"!=a.type||13==a.which)return rcmail_ui.show_attachmentmenu(this,
a),!1}))})):"addressbook"==rcmail.env.task?rcmail.addEventListener("afterupload-photo",function(){rcmail_ui.show_popup("uploadform",!1)}).gui_object("dragmenu","dragmenu"):"settings"==rcmail.env.task&&"folders"==rcmail.env.action&&rcmail_ui.folder_search_init($("#folder-manager"))})};
