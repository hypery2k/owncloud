...
this.init_messageform()):"get"==this.env.action?this.enable_command("download","print",!0):"print"==this.env.action&&this.env.uid&&(bw.safari?setTimeout("window.print()",10):window.print());this.gui_objects.mailboxlist&&(this.env.unread_counts={},this.gui_objects.folderlist=this.gui_objects.mailboxlist,this.http_request("getunread"));this.gui_objects.contactslist&&(this.contact_list=new rcube_list_widget(this.gui_objects.contactslist,{multiselect:!0,draggable:!1,keyboard:!0}),this.contact_list.addEventListener("initrow",
function(a){g.triggerEvent("insertrow",{cid:a.uid,row:a})}).addEventListener("select",function(a){g.compose_recipient_select(a)}).addEventListener("dblclick",function(a){g.compose_add_recipient()}).addEventListener("keypress",function(a){a.key_pressed==a.ENTER_KEY&&(g.compose_add_recipient()||a.last_selected&&"G"==String(a.last_selected).charAt(0)&&$(a.rows[a.last_selected].obj).find("a").first().click())}).init(),$("#_to,#_cc,#_bcc").focus(function(){g.env.focused_field=this}));this.gui_objects.addressbookslist&&
(this.gui_objects.folderlist=this.gui_objects.addressbookslist,this.enable_command("list-adresses",!0));if(this.env.mdn_request&&this.env.uid){var d="sendmdn",e={_uid:this.env.uid,_mbox:this.env.mailbox};confirm(this.get_label("mdnrequest"))||(e._flag="mdnsent",d="mark");this.http_post(d,e)}this.is_framed()||this.env.extwin||this.browser_capabilities_check();break;case "addressbook":this.env.address_group_stack=[];this.gui_objects.folderlist&&(this.env.contactfolders=$.extend($.extend({},this.env.address_sources),
this.env.contactgroups));this.enable_command("add","import",this.env.writable_source);this.enable_command("list","listgroup","pushgroup","popgroup","listsearch","search","reset-search","advanced-search",!0);this.gui_objects.contactslist&&(this.contact_list=new rcube_list_widget(this.gui_objects.contactslist,{multiselect:!0,draggable:this.gui_objects.folderlist?!0:!1,keyboard:!0}),this.contact_list.addEventListener("initrow",function(a){g.triggerEvent("insertrow",{cid:a.uid,row:a})}).addEventListener("keypress",
function(a){g.contactlist_keypress(a)}).addEventListener("select",function(a){g.contactlist_select(a)}).addEventListener("dragstart",function(a){g.drag_start(a)}).addEventListener("dragmove",function(a){g.drag_move(a)}).addEventListener("dragend",function(a){g.drag_end(a)}).init(),$(this.gui_objects.qsearchbox).focusin(function(){g.contact_list.blur()}),this.update_group_commands(),this.command("list"));this.gui_objects.savedsearchlist&&(this.savedsearchlist=new rcube_treelist_widget(this.gui_objects.savedsearchlist,
{id_prefix:"rcmli",id_encode:this.html_identifier_encode,id_decode:this.html_identifier_decode}),this.savedsearchlist.addEventListener("select",function(a){g.triggerEvent("selectfolder",{folder:a.id,prefix:"rcmli"})}));this.set_page_buttons();this.env.cid&&(this.enable_command("show","edit",!0),this.gui_objects.editform&&$("input.groupmember").change(function(){g.group_member_change(this.checked?"add":"del",g.env.cid,g.env.source,this.value)}));this.gui_objects.editform&&(this.enable_command("save",
!0),"add"!=this.env.action&&"edit"!=this.env.action&&"search"!=this.env.action||this.init_contact_form());break;case "settings":this.enable_command("preferences","identities","responses","save","folders",!0);"identities"==this.env.action?this.enable_command("add",2>this.env.identities_level):"edit-identity"==this.env.action||"add-identity"==this.env.action?(this.enable_command("save","edit","toggle-editor",!0),this.enable_command("delete",2>this.env.identities_level),this.editor_init(this.env.editor_config,
"rcmfd_signature")):"folders"==this.env.action?this.enable_command("subscribe","unsubscribe","create-folder","rename-folder",!0):"edit-folder"==this.env.action&&this.gui_objects.editform?(this.enable_command("save","folder-size",!0),parent.rcmail.env.exists=this.env.messagecount,parent.rcmail.enable_command("purge",this.env.messagecount)):"responses"==this.env.action&&this.enable_command("add",!0);this.gui_objects.identitieslist?(this.identity_list=new rcube_list_widget(this.gui_objects.identitieslist,
{multiselect:!1,draggable:!1,keyboard:!0}),this.identity_list.addEventListener("select",function(a){g.identity_select(a)}).addEventListener("keypress",function(a){a.key_pressed==a.ENTER_KEY&&g.identity_select(a)}).init().focus()):this.gui_objects.sectionslist?(this.sections_list=new rcube_list_widget(this.gui_objects.sectionslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.sections_list.addEventListener("select",function(a){g.section_select(a)}).addEventListener("keypress",function(a){a.key_pressed==
...
$("input,select,textarea",this.gui_objects.editform).not(":hidden").not(":disabled").first().select().focus();this.env.contentframe&&!$("#"+this.env.contentframe).is(":visible")&&(this.env.contentframe=null);bw.ie&&$("input[type=file]").keydown(function(a){"13"==a.keyCode&&a.preventDefault()});this.loaded=!0;this.env.lastrefresh=new Date;this.pending_message&&this.display_message(this.pending_message[0],this.pending_message[1],this.pending_message[2]);this.gui_objects.folderlist&&window.rcube_treelist_widget&&
(this.treelist=new rcube_treelist_widget(this.gui_objects.folderlist,{selectable:!0,id_prefix:"rcmli",parent_focus:!0,id_encode:this.html_identifier_encode,id_decode:this.html_identifier_decode,check_droptarget:function(a){return!a.virtual&&g.check_droptarget(a.id)}}),this.treelist.addEventListener("collapse",function(a){g.folder_collapsed(a)}).addEventListener("expand",function(a){g.folder_collapsed(a)}).addEventListener("select",function(a){g.triggerEvent("selectfolder",{folder:a.id,prefix:"rcmli"})}));
...
"_code=0x199")};this.log=function(a){window.console&&console.log&&console.log(a)};this.command=function(a,b,d,e){var f,h,k,l=!1;!d||!d.blur||e&&rcube_event.is_keyboard(e)||d.blur();if(this.busy&&("reset-search"!=a||"search"!=this.last_command))return!1;if(d&&d.href&&0>String(d.href).indexOf("#")&&rcube_event.get_modifier(e))return!0;if(!this.commands[a])return this.is_framed()&&parent.rcmail.command(a,b),!1;if("mail"==this.task&&"compose"==this.env.action&&0>$.inArray(a,this.env.compose_commands)&&
!this.env.server_error){if(this.cmp_hash!=this.compose_field_hash()&&!confirm(this.get_label("notsentwarning")))return!1;this.remove_compose_data(this.env.compose_id);this.compose_skip_unsavedcheck=!0}this.last_command=a;if("function"===typeof this.command_handlers[a])return f=this.command_handlers[a](b,d,e),void 0!==f?f:d?!1:!0;if("string"===typeof this.command_handlers[a])return f=window[this.command_handlers[a]](b,d,e),void 0!==f?f:d?!1:!0;this.triggerEvent("actionbefore",{props:b,action:a,originalEvent:e});
f=this.triggerEvent("before"+a,b||e);if(void 0!==f){if(!1===f)return!1;b=f}f=void 0;switch(a){case "login":this.gui_objects.loginform&&this.gui_objects.loginform.submit();break;case "logout":case "mail":case "addressbook":case "settings":this.switch_task(a);break;case "about":this.redirect("?_task=settings&_action=about",!1);break;case "permaurl":if(d&&d.href&&d.target)return!0;this.env.permaurl&&(parent.location.href=this.env.permaurl);break;case "extwin":if("compose"==this.env.action){if(e=this.gui_objects.messageform,
h=this.open_window(""))this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0,$("input[name='_action']",e).val("compose"),e.action=this.url("mail/compose",{_id:this.env.compose_id,_extwin:1}),e.target=h.name,e.submit()}else this.open_window(this.env.permaurl,!0);break;case "change-format":e=this.env.permaurl+"&_format="+b;"preview"==this.env.action&&(e=e.replace(/_action=show/,"_action=preview")+"&_framed=1");this.env.extwin&&(e+="&_extwin=1");location.href=e;break;case "menu-open":b&&"attachmentmenu"==
...
!1;this.message_list.clear_selection();break;case "expand-all":this.env.autoexpand_threads=1;this.message_list.expand_all();break;case "expand-unread":this.env.autoexpand_threads=2;this.message_list.collapse_all();this.expand_unread();break;case "collapse-all":this.env.autoexpand_threads=0;this.message_list.collapse_all();break;case "nextmessage":this.env.next_uid&&this.show_message(this.env.next_uid,!1,"preview"==this.env.action);break;case "lastmessage":this.env.last_uid&&this.show_message(this.env.last_uid);
break;case "previousmessage":this.env.prev_uid&&this.show_message(this.env.prev_uid,!1,"preview"==this.env.action);break;case "firstmessage":this.env.first_uid&&this.show_message(this.env.first_uid);break;case "compose":e={};if("mail"==this.task)e._mbox=this.env.mailbox,b&&(e._to=b),this.env.search_request&&(e._search=this.env.search_request);else if("addressbook"==this.task)if(b&&0<b.indexOf("@"))e._to=b;else{e=[];b?e.push(b):this.contact_list&&(e=this.contact_list.get_selection());e.length?this.http_post("mailto",
{_cid:e.join(","),_source:this.env.source},!0):this.env.group&&this.http_post("mailto",{_gid:this.env.group,_source:this.env.source},!0);break}else b&&"string"==typeof b?e._to=b:b&&"object"==typeof b&&$.extend(e,b);this.open_compose_step(e);break;case "spellcheck":this.spellcheck_state()?this.editor.spellcheck_stop():this.editor.spellcheck_start();break;case "savedraft":clearTimeout(this.save_timer);if(this.env.draft_id&&this.cmp_hash==this.compose_field_hash()){this.auto_save_start();break}this.submit_messageform(!0);
...
break;case "reply-all":case "reply-list":case "reply":if(h=this.get_single_uid())e={_reply_uid:h,_mbox:this.get_message_mailbox(h)},"reply-all"==a?e._all=!b&&1==this.env.reply_all_mode&&this.commands["reply-list"]?"list":"all":"reply-list"==a&&(e._all="list"),this.open_compose_step(e);break;case "forward-attachment":case "forward-inline":case "forward":h=this.env.uid?[this.env.uid]:this.message_list?this.message_list.get_selection():[];if(h.length){e={_forward_uid:this.uids_to_list(h),_mbox:this.env.mailbox,
_search:this.env.search_request};if("forward-attachment"==a||!b&&this.env.forward_attachment||1<h.length)e._attachment=1;this.open_compose_step(e)}break;case "print":if("get"==this.env.action)this.gui_objects.messagepartframe.contentWindow.print();else if(h=this.get_single_uid())e="&_action=print&_uid="+h+"&_mbox="+urlencode(this.get_message_mailbox(h))+(this.env.safemode?"&_safe=1":""),this.open_window(this.env.comm_path+e,!0,!0)&&"show"!=this.env.action&&this.mark_message("read",h);break;case "viewsource":(h=
...
this.env.qsearch;this.reset_qsearch();this.select_all_mode=!1;if(e&&"compose"==this.env.action)this.contact_list&&this.list_contacts_clear();else if(e&&this.env.mailbox)this.list_mailbox(this.env.mailbox,1);else if(e&&"addressbook"==this.task){if(""==this.env.source){for(m in this.env.address_sources)break;this.env.source=m;this.env.group=""}this.list_contacts(this.env.source,this.env.group,1)}break;case "pushgroup":this.env.address_group_stack.push(b.id),d&&e&&rcube_event.cancel(e);case "listgroup":this.reset_qsearch();
...
_search:this.env.search_request});break;case "export-selected":0<this.contact_list.rowcount&&this.goto_url("export",{_source:this.env.source,_gid:this.env.group,_cid:this.contact_list.get_selection().join(",")});break;case "upload-photo":this.upload_contact_photo(b||this.gui_objects.uploadform);break;case "delete-photo":this.replace_contact_photo("-del-");break;case "preferences":case "identities":case "responses":case "folders":this.goto_url("settings/"+a);break;case "undo":this.http_request("undo",
"",this.display_message("","loading"));break;default:h=a.replace(/-/g,"_"),this[h]&&"function"===typeof this[h]&&(f=this[h](b,d,e))}l||!1!==this.triggerEvent("after"+a,b)||(f=!1);this.triggerEvent("actionafter",{props:b,action:a,aborted:l});return!1===f?!1:d?!1:!0};this.enable_command=function(){var a,b,d=Array.prototype.slice.call(arguments),e=d.pop(),f;for(b=0;b<d.length;b++)if(f=d[b],"string"===typeof f)this.commands[f]=e,this.set_button(f,e?"act":"pas"),this.triggerEvent("enable-command",{command:f,
status:e});else for(a in f)d.push(f[a])};this.command_enabled=function(a){return this.commands[a]};this.set_busy=function(a,b,d){a&&b?(d=this.get_label(b),d==b&&(d="Loading..."),d=this.display_message(d,"loading")):!a&&d&&this.hide_message(d);this.busy=a;this.gui_objects.editform&&this.lock_form(this.gui_objects.editform,a);return d};this.gettext=this.get_label=function(a,b){return b&&this.labels[b+"."+a]?this.labels[b+"."+a]:this.labels[a]?this.labels[a]:a};this.switch_task=function(a){if(this.task!==
a||"mail"==a){var b=this.get_task_url(a);"mail"==a?b+="&_mbox=INBOX":"logout"!=a||this.env.server_error||this.clear_compose_data();this.redirect(b)}};this.get_task_url=function(a,b){b||(b=this.env.comm_path);return b.replace(/_task=[a-z0-9_-]+/i,"_task="+a)};this.reload=function(a){this.is_framed()?parent.rcmail.reload(a):a?setTimeout(function(){g.reload()},a):window.location&&(location.href=this.env.comm_path+(this.env.action?"&_action="+this.env.action:""))};this.add_url=function(a,b,d){d=urlencode(d);
if(/(\?.*)$/.test(a)){var e=RegExp.$1,f=RegExp("((\\?|&)"+RegExp.escape(b)+"=[^&]*)"),e=f.test(e)?e.replace(f,RegExp.$2+b+"="+d):e+("&"+b+"="+d);return a.replace(/(\?.*)$/,e)}return a+"?"+b+"="+d};this.is_framed=function(){return this.env.framed&&parent.rcmail&&parent.rcmail!=this&&"function"==typeof parent.rcmail.command};this.save_pref=function(a){var b={_name:a.name,_value:a.value};a.session&&(b._session=a.session);a.env&&(this.env[a.env]=a.value);this.http_post("save-pref",b)};this.html_identifier=
...
function(a){var b="addressbook"==this.env.task?"collapsed_abooks":"collapsed_folders";if(a.collapsed)this.env[b]=this.env[b]+"&"+urlencode(a.id)+"&",!a.virtual&&this.env.mailbox&&this.env.mailbox.startsWith(name+this.env.delimiter)&&this.command("list",name);else{var d=RegExp("&"+urlencode(a.id)+"&");this.env[b]=this.env[b].replace(d,"")}this.drag_active||(this.command("save-pref",{name:b,value:this.env[b]}),this.env.unread_counts&&this.set_unread_count_display(a.id,!1))};this.doc_mouse_up=function(a){var b,
d=rcube_event.get_target(a);if(!$(d).closest(".ui-dialog, .ui-widget-overlay").length){window.rcube_list_widget&&rcube_list_widget._instances.length&&$.each(rcube_list_widget._instances,function(b,d){d&&!rcube_mouse_is_over(a,d.list.parentNode)&&d.blur()});if(this.buttons_sel){for(b in this.buttons_sel)"function"!==typeof b&&this.button_out(this.buttons_sel[b],b);this.buttons_sel={}}setTimeout(function(a){var b,h,k,l,n=$(d).parents();for(l=g.menu_stack.length-1;0<=l;l--)k=g.menu_stack[l],b=$("#"+
k),!b.is(":visible")||d==b.data("opener")||d==b.get(0)||n.is(b.data("opener"))||k==h||"true"==b.attr("data-editable")&&$(d).parents("#"+k).length||"true"==b.attr("data-sticky")&&rcube_mouse_is_over(a,b.get(0))||g.hide_menu(k,a),h=b.data("parent")},10,a)}};this.doc_keypress=function(a){var b=function(a){var b,d=0>a?"prevAll":"nextAll",e=0>a?"last":"first";return g.focused_menu&&(b=$("#"+g.focused_menu))?(a=b.find(":focus").closest("li")[d](":has(:not([aria-disabled=true]))").find("a,input")[e](),a.length||
(a=b.find(":focus").closest("ul")[d](":has(:not([aria-disabled=true]))").find("a,input")[e]()),a.focus().length):0},d=a.target||{},e=rcube_event.get_keycode(a);rcube_event._last_keyboard_event=a;if(27!=a.keyCode&&(!this.menu_keyboard_active||"TEXTAREA"==d.nodeName||"SELECT"==d.nodeName))return!0;switch(e){case 38:case 40:case 63232:case 63233:return b(38==e||63232==e?-1:1),rcube_event.cancel(a);case 9:return this.focused_menu&&(d=rcube_event.get_modifier(a),b(d==SHIFT_KEY?-1:1)||this.hide_menu(this.focused_menu,
...
{uid:d,row:a})};this.add_message_row=function(a,b,d,e){if(!this.gui_objects.messagelist||!this.message_list||d.mbox!=this.env.mailbox&&!d.skip_mbox_check)return!1;this.env.messages[a]||(this.env.messages[a]={});$.extend(this.env.messages[a],{deleted:d.deleted?1:0,replied:d.answered?1:0,unread:d.seen?0:1,forwarded:d.forwarded?1:0,flagged:d.flagged?1:0,has_children:d.has_children?1:0,depth:d.depth?d.depth:0,unread_children:d.unread_children?d.unread_children:0,parent_uid:d.parent_uid?d.parent_uid:0,
selected:this.select_all_mode||this.message_list.in_selection(a),ml:d.ml?1:0,ctype:d.ctype,mbox:d.mbox,flags:d.extra_flags});var f,h,k,g="",n="",m="",p="",q=this.message_list,v=q.rows;k=this.env.messages[a];var s=this.html_identifier(a,!0),t="message"+(d.seen?"":" unread")+(d.deleted?" deleted":"")+(d.flagged?" flagged":"")+(k.selected?" selected":""),w={cols:[],style:{},id:"rcmrow"+s,uid:a};f="msgicon";null===this.env.status_col&&(f+=" status",d.deleted?(g+=" deleted",n+=this.get_label("deleted")+
" "):d.seen?0<d.unread_children&&(g+=" unreadchildren"):(g+=" unread",n+=this.get_label("unread")+" "));d.answered&&(g+=" replied",n+=this.get_label("replied")+" ");d.forwarded&&(g+=" forwarded",n+=this.get_label("replied")+" ");k.selected&&!q.in_selection(a)&&q.selection.push(a);this.env.threading&&(k.depth?(m+='<span id="rcmtab'+s+'" class="branch" style="width:'+15*k.depth+'px;">&nbsp;&nbsp;</span>',v[k.parent_uid]&&!1===v[k.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||
v[k.parent_uid]&&v[k.parent_uid].expanded)?(w.style.display="none",k.expanded=!1):k.expanded=!0,t+=" thread expanded"):k.has_children&&(void 0===k.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&k.unread_children)&&(k.expanded=!0),p='<div id="rcmexpando'+w.id+'" class="'+(k.expanded?"expanded":"collapsed")+'">&nbsp;&nbsp;</div>',t+=" thread"+(k.expanded?" expanded":"")),d.unread_children&&d.seen&&!k.expanded&&(t+=" unroot"));m+='<span id="msgicn'+w.id+'" class="'+f+g+'" title="'+
n+'"></span>';w.className=t;b.subject&&(f=d.mbox==this.env.drafts_mailbox?"compose":"show",n={_mbox:d.mbox},n[d.mbox==this.env.drafts_mailbox?"_draft_uid":"_uid"]=a,b.subject='<a href="'+this.url(f,n)+'" onclick="return rcube_event.keyboard_only(event)" onmouseover="rcube_webmail.long_subject_title(this,'+(k.depth+1)+')" tabindex="-1"><span>'+b.subject+"</span></a>");for(h in this.env.listcols)f=this.env.listcols[h],a={className:String(f).toLowerCase(),events:{}},this.env.coltypes[f]&&this.env.coltypes[f].hidden&&
(a.className+=" hidden"),"flag"==f?(f=d.flagged?"flagged":"unflagged",k=this.get_label(f),f='<span id="flagicn'+w.id+'" class="'+f+'" title="'+k+'"></span>'):"attachment"==f?(k=this.get_label("withattachment"),f=d.attachmentClass?'<span class="'+d.attachmentClass+'" title="'+k+'"></span>':/application\/|multipart\/(m|signed)/.test(d.ctype)?'<span class="attachment" title="'+k+'"></span>':/multipart\/report/.test(d.ctype)?'<span class="report"></span>':"&nbsp;"):"status"==f?(k="",d.deleted?(f="deleted",
k=this.get_label("deleted")):d.seen?f=0<d.unread_children?"unreadchildren":"msgicon":(f="unread",k=this.get_label("unread")),f='<span id="statusicn'+w.id+'" class="'+f+g+'" title="'+k+'"></span>'):"threads"==f?f=p:"subject"==f?(bw.ie&&(a.events.mouseover=function(){rcube_webmail.long_subject_title_ex(this)}),f=m+b[f]):"priority"==f?0<d.prio&&6>d.prio?(k=this.get_label("priority")+" "+d.prio,f='<span class="prio'+d.prio+'" title="'+k+'"></span>'):f="&nbsp;":f="folder"==f?'<span onmouseover="rcube_webmail.long_subject_title(this)">'+
...
b||this.env.sort_order!=d)f=1,this.set_list_sorting(b,d);this.env.threading!=e&&(f=1,h._threads=e);if(a&&a.length){var g,l,n=[],m=this.env.listcols;for(e=0;e<m.length;e++)l=m[e],g=$.inArray(l,a),-1!=g&&(n.push(l),delete a[g]);for(e=0;e<a.length;e++)a[e]&&n.push(a[e]);n.join()!=m.join()&&(f=1,h._cols=n.join(","))}f&&this.list_mailbox("","",b+"_"+d,h)};this.show_message=function(a,b,d){if(a){var e,f=window,h="&_action="+(d?"preview":"show")+"&_uid="+a+"&_mbox="+urlencode(this.get_message_mailbox(a));
...
this.message_list.rows,f=e[a],h=e[a].depth,k=[];f.depth?f.unread&&(a=this.message_list.find_root(a),e[a].unread_children--,this.set_unread_children(a)):d--;a=f.parent_uid;for(f=f.obj.nextSibling;f;){if(1==f.nodeType&&(b=e[f.uid])){if(!b.depth||b.depth<=h)break;b.depth--;$("#rcmtab"+b.id).width(15*b.depth).html("");b.depth?(b.depth==h&&(b.parent_uid=a),b.unread&&k.length&&k[k.length-1].unread_children++):(d++,b.parent_uid=0,b.has_children&&($("#"+b.id+" .leaf:first").attr("id","rcmexpando"+b.id).attr("class",
"none"!=b.obj.style.display?"expanded":"collapsed").bind("mousedown",{uid:b.uid},function(a){return g.expand_message_row(a,a.data.uid)}),b.unread_children=0,k.push(b)),"none"==b.obj.style.display&&$(b.obj).show())}f=f.nextSibling}for(b=0;b<k.length;b++)this.set_unread_children(k[b].uid);return d};this.delete_excessive_thread_rows=function(){for(var a=this.message_list.rows,b=this.message_list.tbody.firstChild,d=this.env.pagesize+1;b;)1==b.nodeType&&(r=a[b.uid])&&(!r.depth&&d&&d--,d||this.message_list.remove_row(b.uid)),
b=b.nextSibling};this.set_message_icon=function(a){var b="",d=this.message_list.rows[a];if(!d)return!1;d.icon&&(a="msgicon",d.deleted?(a+=" deleted",b+=this.get_label("deleted")+" "):d.unread?(a+=" unread",b+=this.get_label("unread")+" "):d.unread_children&&(a+=" unreadchildren"),d.msgicon==d.icon&&(d.replied&&(a+=" replied",b+=this.get_label("replied")+" "),d.forwarded&&(a+=" forwarded",b+=this.get_label("forwarded")+" "),a+=" status"),$(d.icon).attr("class",a).attr("title",b));d.msgicon&&d.msgicon!=
d.icon&&(b="",a="msgicon",!d.unread&&d.unread_children&&(a+=" unreadchildren"),d.replied&&(a+=" replied",b+=this.get_label("replied")+" "),d.forwarded&&(a+=" forwarded",b+=this.get_label("forwarded")+" "),$(d.msgicon).attr("class",a).attr("title",b));d.flagicon&&(a=d.flagged?"flagged":"unflagged",b=this.get_label(a),$(d.flagicon).attr("class",a).attr("aria-label",b).attr("title",b))};this.set_message_status=function(a,b,d){var e=this.message_list.rows[a];if(!e)return!1;"unread"==b&&e.unread!=d&&this.update_thread_root(a,
d?"unread":"read");-1<$.inArray(b,["unread","deleted","replied","forwarded","flagged"])&&(e[b]=d)};this.set_message=function(a,b,d){var e=this.message_list&&this.message_list.rows[a];if(!e)return!1;b&&this.set_message_status(a,b,d);if(-1<$.inArray(b,["unread","deleted","flagged"]))$(e.obj)[e[b]?"addClass":"removeClass"](b);this.set_unread_children(a);this.set_message_icon(a)};this.set_unread_children=function(a){a=this.message_list.rows[a];a.parent_uid||(a.unread||!a.unread_children||a.expanded?$(a.obj).removeClass("unroot"):
$(a.obj).addClass("unroot"))};this.copy_messages=function(a,b){if(a&&"object"===typeof a)a=a.id;else if(!a)return this.folder_selector(b,function(a){g.command("copy",a)});if(a&&a!=this.env.mailbox){var d=this.selection_post_data({_target_mbox:a});d._uid&&this.http_post("copy",d,this.display_message(this.get_label("copyingmessage"),"loading"))}};this.move_messages=function(a,b){if(a&&"object"===typeof a)a=a.id;else if(!a)return this.folder_selector(b,function(a){g.command("move",a)});if(a&&(a!=this.env.mailbox||
this.is_multifolder_listing())){var d=!1,e=this.selection_post_data({_target_mbox:a});e._uid&&("show"==this.env.action?d=this.set_busy(!0,"movingmessage"):this.show_contentframe(!1),this.enable_command(this.env.message_commands,!1),this._with_selected_messages("move",e,d))}};this.delete_messages=function(a){var b=this.message_list,d=this.env.trash_mailbox;if(this.env.flag_for_deletion)return this.mark_message("delete"),!1;d&&this.env.mailbox!=d?this.env.delete_junk&&this.env.junk_mailbox&&this.env.mailbox==
this.env.junk_mailbox?this.permanently_remove_messages():b&&b.modkey==SHIFT_KEY||a&&rcube_event.get_modifier(a)==SHIFT_KEY?confirm(this.get_label("deletemessagesconfirm"))&&this.permanently_remove_messages():this.move_messages(d):this.permanently_remove_messages();return!0};this.permanently_remove_messages=function(){var a=this.selection_post_data();a._uid&&(this.show_contentframe(!1),this._with_selected_messages("delete",a))};this._with_selected_messages=function(a,b,d){var e=0,f="delete"==a||!this.is_multifolder_listing();
if(this.message_list){var h,g,l,n=[],m=this.message_list.get_selection();h=0;for(len=m.length;h<len;h++)g=m[h],this.env.threading&&(e+=this.update_thread(g),l=this.message_list.find_root(g),l!=g&&0>$.inArray(l,n)&&n.push(l)),f&&this.message_list.remove_row(g,this.env.display_next&&h==m.length-1);!this.env.display_next&&f&&this.message_list.clear_selection();h=0;for(len=n.length;h<len;h++)this.add_tree_icons(n[h])}0>e?b._count=-1*e:0<e&&f&&this.delete_excessive_thread_rows();f||(b._refresh=1);d||(d=
...
this.mark_message=function(a,b){var d=[],e=[],f,h,g,l=this.message_list;b?d[0]=b:this.env.uid?d[0]=this.env.uid:l&&(d=l.get_selection());if(l)for(l.focus(),h=0,f=d.length;h<f;h++)g=d[h],("read"==a&&l.rows[g].unread||"unread"==a&&!l.rows[g].unread||"delete"==a&&!l.rows[g].deleted||"undelete"==a&&l.rows[g].deleted||"flagged"==a&&!l.rows[g].flagged||"unflagged"==a&&l.rows[g].flagged)&&e.push(g);else e=d;if(e.length||this.select_all_mode)switch(a){case "read":case "unread":this.toggle_read_status(a,e);
...
_flag:a}),h=this.display_message(this.get_label("markingmessage"),"loading");for(d=0;d<e;d++)this.set_message(b[d],"flagged","flagged"==a?!0:!1);this.http_post("mark",f,h)};this.toggle_delete_status=function(a){var b=a.length,d,e,f=!0,h=this.message_list?this.message_list.rows:{};if(1==b)return!this.message_list||h[a[0]]&&!h[a[0]].deleted?this.flag_as_deleted(a):this.flag_as_undeleted(a),!0;for(d=0;d<b;d++)if(e=a[d],h[e]&&!h[e].deleted){f=!1;break}f?this.flag_as_undeleted(a):this.flag_as_deleted(a);
...
{},h=0,g=0,l=a.length;g<l;g++)uid=a[g],f[uid]&&(f[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(h+=this.update_thread(uid),this.message_list.remove_row(uid,this.env.display_next&&g==this.message_list.selection.length-1)):this.set_message(uid,"deleted",!0));this.env.skip_deleted&&this.message_list&&(this.env.display_next||this.message_list.clear_selection(),0>h?d._count=-1*h:0<h&&this.delete_excessive_thread_rows());b.length&&(d._ruid=this.uids_to_list(b));this.env.skip_deleted&&this.env.display_next&&
this.env.next_uid&&(d._next_uid=this.env.next_uid);this.http_post("mark",d,e)};this.flag_deleted_as_read=function(a){var b,d,e,f=this.message_list?this.message_list.rows:{};"string"==typeof a&&(a=a.split(","));d=0;for(e=a.length;d<e;d++)b=a[d],f[b]&&this.set_message(b,"unread",!1)};this.uids_to_list=function(a){return this.select_all_mode?"*":1>=a.length?a.join(","):a};this.set_button_titles=function(){var a="deletemessage";this.env.flag_for_deletion||!this.env.trash_mailbox||this.env.mailbox==this.env.trash_mailbox||
this.env.delete_junk&&this.env.junk_mailbox&&this.env.mailbox==this.env.junk_mailbox||(a="movemessagetotrash");this.set_alttext("delete",a)};this.expunge_mailbox=function(a){var b,d={_mbox:a};a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1,this.env.search_request&&(d._search=this.env.search_request));this.http_post("expunge",d,b)};this.purge_mailbox=function(a){var b,d={_mbox:a};if(!confirm(this.get_label("purgefolderconfirm")))return!1;a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),
d._reload=1);this.http_post("purge",d,b)};this.purge_mailbox_test=function(){return this.env.exists&&(this.env.mailbox==this.env.trash_mailbox||this.env.mailbox==this.env.junk_mailbox||this.env.mailbox.startsWith(this.env.trash_mailbox+this.env.delimiter)||this.env.mailbox.startsWith(this.env.junk_mailbox+this.env.delimiter))};this.login_user_keyup=function(a){var b=rcube_event.get_keycode(a),d=$("#rcmloginpwd");return 13==b&&d.length&&!d.val()?(d.focus(),rcube_event.cancel(a)):!0};this.open_compose_step=
function(a){a=this.url("mail/compose",a);this.env.compose_extwin&&!this.env.extwin?this.open_window(a):(this.redirect(a),this.env.extwin&&window.resizeTo(Math.max(this.env.popup_width,$(window).width()),$(window).height()+24))};this.init_messageform=function(){if(!this.gui_objects.messageform)return!1;var a,b=$("[name='_from']"),d=$("[name='_to']"),e=$("input[name='_subject']"),f=$("[name='_message']").get(0),h="1"==$("input[name='_is_html']").val(),g=["cc","bcc","replyto","followupto"],l,n=this.opener();
n&&"compose"==n.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():n.redirect(n.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(l={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(d,l);for(a in g)this.init_address_input_events($("[name='_"+g[a]+"']"),l);h||(a=this.env.top_posting?0:f.value.length,this.set_caret_pos(f,a),"select-one"==b.prop("type")&&this.change_identity(b[0]),
a&&$(f).scrollTop(f.scrollHeight));this.env.save_localstorage&&this.compose_restore_dialog(0,h);""==d.val()?d.focus():""==e.val()?e.focus():f&&f.focus();this.env.compose_focus_elem=document.activeElement;this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e,f,h=this.local_storage_get_item("compose.index",[]),k=function(a){++a<h.length&&g.compose_restore_dialog(a,b)};for(d=a||0;d<h.length;d++)if(e=h[d],f=this.local_storage_get_item("compose."+e,null,
!0)){if(f.changed&&e==this.env.compose_id){this.restore_compose_form(e,b);break}if(!(this.env.draft_id&&f.draft_id&&f.draft_id!=this.env.draft_id||this.env.reply_msgid&&f.reply_msgid!=this.env.reply_msgid)&&f.changed&&f.session!=this.env.session_id){this.show_popup_dialog(this.get_label("restoresavedcomposedata").replace("$date",(new Date(f.changed)).toLocaleString()).replace("$subject",f._subject).replace(/\n/g,"<br/>"),this.get_label("restoremessage"),[{text:this.get_label("restore"),click:function(){g.restore_compose_form(e,
b);g.remove_compose_data(e);g.save_compose_form_local();$(this).dialog("close")}},{text:this.get_label("delete"),click:function(){g.remove_compose_data(e);$(this).dialog("close");k(d)}},{text:this.get_label("ignore"),click:function(){$(this).dialog("close");k(d)}}]);break}}};this.init_address_input_events=function(a,b){this.env.recipients_delimiter=this.env.recipients_separator+" ";a.keydown(function(a){return g.ksearch_keydown(a,this,b)}).attr({autocomplete:"off","aria-autocomplete":"list","aria-expanded":"false",
role:"combobox"})};this.submit_messageform=function(a){var b=this.gui_objects.messageform;if(b){var d=this.set_busy(!0,a?"savingmessage":"sendingmessage"),e=this.spellcheck_lang(),f=[];$("li",this.gui_objects.attachmentlist).each(function(){f.push(this.id.replace(/^rcmfile/,""))});$('input[name="_attachments"]',b).val(f.join());b.target="savetarget";b._draft.value=a?"1":"";b.action=this.add_url(b.action,"_unlock",d);b.action=this.add_url(b.action,"_lang",e);b.action=this.add_url(b.action,"_framed",
1);this.submit_timer=setTimeout(function(){g.set_busy(!1,null,d);g.display_message(g.get_label("requesttimedout"),"error")},1E3*this.env.request_timeout);b.submit()}};this.compose_recipient_select=function(a){var b,d,e=0;for(d=0;d<a.selection.length;d++)b=a.selection[d],this.env.contactdata[b]&&e++;this.enable_command("add-recipient",e)};this.compose_add_recipient=function(a){a||(a=$(this.env.focused_field).filter(":visible"),a=a.length?a.attr("id").replace("_",""):"to");var b=[],d=$("#_"+a),e=this.env.recipients_delimiter;
...
!h.test(f)&&(f+=e+" "),d.val(f+b.join(e+" ")+e+" "),this.triggerEvent("add-recipient",{field:a,recipients:b}));return b.length};this.check_compose_input=function(a){var b=$("[name='_to']"),d=$("[name='_cc']"),e=$("[name='_bcc']"),f=$("[name='_from']"),h=$("[name='_subject']");if("text"==f.prop("type")&&!rcube_check_email(f.val(),!0))return alert(this.get_label("nosenderwarning")),f.focus(),!1;d=b.val()?b.val():d.val()?d.val():e.val();if(!rcube_check_email(d.replace(/^\s+/,"").replace(/[\s,;]+$/,""),
...
$.each(["replyto","bcc"],function(){var a,b=e&&g.env.identities[e]?g.env.identities[e][this]:"",n=d&&g.env.identities[d]?g.env.identities[d][this]:"",m=$('[name="_'+this+'"]'),p=m.val();b&&p&&(a=RegExp("\\s*"+RegExp.escape(b)+"\\s*"),p=p.replace(a,""));a=RegExp(h+"\\s*"+h,"g");p=String(p).replace(a,f);a=RegExp("^[\\s"+h+"]+");p=p.replace(a,"");n&&-1==p.indexOf(n)&&-1==p.indexOf(n.replace(/"/g,""))&&(p&&(a=RegExp("["+h+"\\s]+$"),p=p.replace(a,"")+f+" "),p+=n+f+" ");(b||n)&&m.val(p).change()});this.editor.change_signature(d,
...
num:d.length})))}};this.ksearch_query_results=function(a,b,d){this.multi_thread_http_response(a,d);if(this.ksearch_value&&(!this.ksearch_input||b==this.ksearch_value)){var e,f,h,k,l,n=this.ksearch_value,m=this.env.autocomplete_max?this.env.autocomplete_max:15;this.ksearch_pane||(b=$("<ul>"),this.ksearch_pane=$("<div>").attr("id","rcmKSearchpane").attr("role","listbox").css({position:"absolute","z-index":3E4}).append(b).appendTo(document.body),this.ksearch_pane.__ul=b[0]);b=this.ksearch_pane.__ul;
d&&this.ksearch_pane.data("reqid")==d?m-=b.childNodes.length:(this.ksearch_pane.data("reqid",d),b.innerHTML="",this.env.contacts=[],e=$(this.ksearch_input).offset(),this.ksearch_pane.css({left:e.left+"px",top:e.top+this.ksearch_input.offsetHeight+"px",display:"none"}));if(a&&(h=a.length))for(e=0;e<h&&0<m;e++)k="object"===typeof a[e]?a[e].display||a[e].name:a[e],l="object"===typeof a[e]?a[e].type:"",f=e+this.env.contacts.length,$("<li>").attr("id","rcmkSearchItem"+f).attr("role","option").html('<i class="icon"></i>'+
this.quote_html(k.replace(RegExp("("+RegExp.escape(n)+")","ig"),"##$1%%")).replace(/##([^%]+)%%/g,"<b>$1</b>")).addClass(l||"").appendTo(b).mouseover(function(){g.ksearch_select(this)}).mouseup(function(){g.ksearch_click(this)}).get(0)._rcm_id=f,m-=1;b.childNodes.length&&($(this.ksearch_input).attr("aria-haspopup","true").attr("aria-expanded","true").attr("aria-owns","rcmKSearchpane"),this.ksearch_pane.show(),this.env.contacts.length||this.ksearch_select($("li:first",b).get(0)));h&&(this.env.contacts=
...
function(a){this.preview_timer&&clearTimeout(this.preview_timer);var b,d,e,f,h=!1,k=this.env.source?this.env.address_sources[this.env.source]:null;(d=a.get_single_selection())?this.preview_timer=setTimeout(function(){g.load_contact(d,"show")},200):this.env.contentframe&&this.show_contentframe(!1);if(a.selection.length){a.draggable=!1;this.env.selection_sources=[];k&&this.env.selection_sources.push(this.env.source);for(b in a.selection)f=a.data[a.selection[b]],k?h=h||!k.readonly&&!f.readonly:(e=String(a.selection[b]).replace(/^[^-]+-/,
...
"move",0<a.selection.length&&h);return!1};this.list_contacts=function(a,b,d){var e,f={},h=void 0===a&&void 0===b&&void 0===d,g=window;a||(a=this.env.source);if(d&&this.current_page==d&&a==this.env.source&&b==this.env.group)return!1;a!=this.env.source?(d=this.env.current_page=1,this.reset_qsearch()):h||b==this.env.group||(d=this.env.current_page=1);this.env.search_id?e="S"+this.env.search_id:this.env.search_request||(e=b?"G"+a+b:a);this.env.source=a;this.env.group=b;h=$.inArray(this.env.group,this.env.address_group_stack);
this.env.address_group_stack=0>h?[]:this.env.address_group_stack.slice(0,h);this.env.group?(this.env.address_group_stack.push(this.env.group),e="G"+a+this.env.address_group_stack[0]):this.gui_objects.addresslist_title&&$(this.gui_objects.addresslist_title).html(this.get_label("contacts"));this.env.search_id||this.select_folder(e,"",!0);if(this.gui_objects.contactslist)this.list_contacts_remote(a,b,d);else{if(e=this.get_frame_window(this.env.contentframe))g=e,f._framed=1;b&&(f._gid=b);d&&(f._page=
...
{};this.contact_list.clear(!0);this.show_contentframe(!1);this.enable_command("delete","move","copy",!1);this.enable_command("compose",this.env.group?!0:!1)};this.set_group_prop=function(a){if(this.gui_objects.addresslist_title){var b=$(this.gui_objects.addresslist_title).html("");1<this.env.address_group_stack.length&&($('<a href="#list">...</a>').attr("title",this.gettext("uponelevel")).addClass("poplink").appendTo(b).click(function(a){return g.command("popgroup","",this)}),b.append("&nbsp;&raquo;&nbsp;"));
b.append($("<span>").text(a.name))}this.triggerEvent("groupupdate",a)};this.load_contact=function(a,b,d){var e,f={},h=window,g=this.contact_list?this.contact_list.data[a]:null;if(e=this.get_frame_window(this.env.contentframe))f._framed=1,h=e,this.show_contentframe(!0),a||this.contact_list.clear_selection(),this.enable_command("compose",g&&g.email),this.enable_command("export-selected",g&&"group"!=g._type);else if(d)return!1;!b||!a&&"add"!=b||this.drag_active||(this.env.group&&(f._gid=this.env.group),
this.env.search_request&&(f._search=this.env.search_request),f._action=b,f._source=this.env.source,f._cid=a,this.location_href(f,h,!0));return!0};this.group_member_change=function(a,b,d,e){"add"!=a&&(a="del");var f=this.get_label("add"==a?"addingmember":"removingmember"),f=this.display_message(f,"loading");this.http_post("group-"+a+"members",{_cid:b,_source:d,_gid:e},f)};this.contacts_drag_menu=function(a,b){var d="group"==b.type?b.source:b.id,e=this.env.source;if(!this.env.address_sources[d]||this.env.address_sources[d].readonly)return!0;
...
b=this.env.coltypes[a];var k="ff_"+a+(b.count||0),f=$("<div>").addClass("row"),l=$("<div>").addClass("contactfieldcontent data"),n=$("<div>").addClass("contactfieldlabel label");b.subtypes_select?n.html(b.subtypes_select):n.html('<label for="'+k+'">'+b.label+"</label>");var m=1!=b.limit?"[]":"";if("text"==b.type||"date"==b.type)h=$("<input>").addClass("ff_"+a).attr({type:"text",name:"_"+a+m,size:b.size,id:k}).appendTo(l),this.init_edit_field(a,h),"date"==b.type&&$.datepicker&&h.datepicker();else if("textarea"==
b.type)h=$("<textarea>").addClass("ff_"+a).attr({name:"_"+a+m,cols:b.size,rows:b.rows,id:k}).appendTo(l),this.init_edit_field(a,h);else if("composite"==b.type){var p,q,v=[],s=[];if(h=this.env[a+"_template"])for(k=0;k<h.length;k++)v.push(h[k][1]),s.push(h[k][2]);else for(p in b.childs)v.push(p);for(k=0;k<v.length;k++)p=v[k],h=b.childs[p],h=$("<input>").addClass("ff_"+p).attr({type:"text",name:"_"+p+m,size:h.size}).appendTo(l),l.append(s[k]||" "),this.init_edit_field(p,h),q||(q=h);h=q}else if("select"==
b.type){h=$("<select>").addClass("ff_"+a).attr({name:"_"+a+m,id:k}).appendTo(l);var t=h.attr("options");t[t.length]=new Option("---","");b.options&&$.each(b.options,function(a,b){t[t.length]=new Option(b,a)})}h&&($('<a href="#del"></a>').addClass("contactfieldbutton deletebutton").attr({title:this.get_label("delete"),rel:a}).html(this.env.delbutton).click(function(){g.delete_edit_field(this);return!1}).appendTo(l),f.append(n).append(l).appendTo(e.show()),h.first().focus(),b.count||(b.count=0),++b.count==
...
function(a){a.query&&g.subscription_select()}).draggable({cancel:"li.mailbox.root"}).droppable({accept:function(a){a=g.folder_id2name($(a).attr("id"));var d=g.folder_id2name(this.id),e=g.env.subscriptionrows[a];return e&&!e[2]&&d!=a.replace(g.last_sub_rx,"")&&!d.startsWith(a+g.env.delimiter)},drop:function(a,d){var e=g.folder_id2name(d.draggable.attr("id")),f=g.folder_id2name(this.id);g.subscription_move_folder(e,f)}})};this.folder_id2name=function(a){return g.html_identifier_decode(a.replace(/^rcmli/,
""))};this.subscription_select=function(a){var b;a&&"*"!=a&&(b=this.env.subscriptionrows[a])?(this.env.mailbox=a,this.show_folder(a),this.enable_command("delete-folder",!b[2])):(this.env.mailbox=null,this.show_contentframe(!1),this.enable_command("delete-folder","purge",!1))};this.subscription_move_folder=function(a,b){if(a&&null!==b&&a!=b&&b!=a.replace(this.last_sub_rx,"")){var d=a.split(this.env.delimiter).pop(),d=""===b||"*"===b?d:b+this.env.delimiter+d;d!=a&&this.http_post("rename-folder",{_folder_oldname:a,
_folder_newname:d},this.set_busy(!0,"foldermoving"))}};this.create_folder=function(){this.show_folder("",this.env.mailbox)};this.delete_folder=function(a){a||(a=this.env.mailbox);a&&confirm(this.get_label("deletefolderconfirm"))&&this.http_post("delete-folder",{_mbox:a},this.set_busy(!0,"folderdeleting"))};this.add_folder_row=function(a,b,d,e,f,h,k,l){if(!this.gui_objects.subscriptionlist)return!1;this.subscription_list.is_search()&&(this.subscription_select(),this.subscription_list.reset_search());
this.subscription_list.draggable("destroy").droppable("destroy");var n,m,p,q,v,s="",t=[],w=[],u=[],x=$(this.gui_objects.subscriptionlist);n=k?k:$($("li",x).get(1)).clone(!0);if(!n.length)return this.goto_url("folders"),!1;n.attr({id:"rcmli"+this.html_identifier_encode(a),"class":h});k&&k.length||($("ul,div.treetoggle",n).remove(),n.removeData("filtered"));$("a:first",n).text(d);$('input[name="_subscribed[]"]:first',n).val(a).prop({checked:f?!0:!1,disabled:e?!0:!1});this.env.subscriptionrows[a]=[b,
d,!1];$.each(this.env.subscriptionrows,function(a,b){b[3]=a;t.push(b)});try{v=new Intl.Collator(this.env.locale.replace("_","-"))}catch(y){}t.sort(function(a,b){var d,e,f,h=a[0].split(g.env.delimiter),k=b[0].split(g.env.delimiter),l=h.length;for(d=0;d<l;d++){e=h[d];f=k[d];if(e!==f)return void 0===f?1:v?v.compare(e,f):e<f?-1:1;if(d==l-1)return-1}});for(m in t)d=t[m][3],t[m][2]?(b=d+this.env.delimiter,b!=this.env.prefix_ns&&(u.push(d),p=b)):p&&d.startsWith(p)?u.push(d):(w.push(d),p=null);for(m=0;m<
u.length;m++)a.startsWith(u[m]+this.env.delimiter)&&(q=u[m]);for(m=0;!q&&m<w.length;m++)m&&w[m]==a&&(q=w[m-1]);if(q&&(m=this.subscription_list.get_item(q,!0))){if(p=a.lastIndexOf(this.env.delimiter))s=a.substring(0,p),s=this.subscription_list.get_item(s,!0),$("div.treetoggle",s).length||$("<div>&nbsp;</div>").addClass("treetoggle collapsed").appendTo(s),$("ul",s).length||$("<ul>").css("display","none").appendTo(s);if(s&&m==s)$("ul:first",s).append(n);else{for(;(d=$(m).parent().parent().get(0))&&(!s||
d!=s)&&$(d).is("li.mailbox");)m=d;$(m).after(n)}}else x.append(n);$.extend(this.env.subscriptionrows,l||{});this.subscription_list.reset(!0);this.subscription_select();s&&this.subscription_list.expand(this.folder_id2name(s.id));n=n.show().get(0);n.scrollIntoView&&n.scrollIntoView();return n};this.replace_folder_row=function(a,b,d,e,f,h){if(!this.gui_objects.subscriptionlist)return this.is_framed()?window.parent.rcmail.replace_folder_row(a,b,d,e,f,h):!1;this.subscription_list.is_search()&&(this.subscription_select(),
this.subscription_list.reset_search());var k={},l=this.subscription_list.get_item(a,!0),n=$(l).parent(),m=a.length,p=this.env.subscriptionrows[a][0].length,q=$('input[name="_subscribed[]"]:first',l).prop("checked");a==b?$(l).attr("class",h||""):($("li",l).each(function(){var a=g.folder_id2name(this.id),e=g.env.subscriptionrows[a],f=b+a.slice(m);this.id="rcmli"+g.html_identifier_encode(f);$('input[name="_subscribed[]"]:first',this).val(f);e[0]=d+e[0].slice(p);k[f]=e;delete g.env.subscriptionrows[a]}),
l=$(l).detach(),delete this.env.subscriptionrows[a],n.get(0)==this.gui_objects.subscriptionlist||$("li",n).length||$("ul,div.treetoggle",n.parent()).remove(),this.add_folder_row(b,d,e,f,q,h,l,k))};this.remove_folder_row=function(a){this.subscription_list.is_search()&&(this.subscription_select(),this.subscription_list.reset_search());var b=[],d=this.subscription_list.get_item(a,!0);$("li",d).each(function(){b.push(g.folder_id2name(this.id))});this.subscription_list.remove(a);b.push(a);$.each(b,function(a,
...
"pas"==b||"sel"==b?"-1":e.attr("data-tabindex")||"0").attr("aria-disabled","pas"==b||"sel"==b?"true":"false"))};this.set_alttext=function(a,b){var d,e,f,h,g=this.buttons[a],l=g?g.length:0;for(d=0;d<l;d++)e=g[d],f=document.getElementById(e.id),"image"==e.type&&f?(f.setAttribute("alt",this.get_label(b)),(h=f.parentNode)&&"a"==h.tagName.toLowerCase()&&h.setAttribute("title",this.get_label(b))):f&&f.setAttribute("title",this.get_label(b))};this.button_over=function(a,b){this.button_event(a,b,"over")};
this.button_sel=function(a,b){this.button_event(a,b,"sel")};this.button_out=function(a,b){this.button_event(a,b,"act")};this.button_event=function(a,b,d){var e,f,h,g=this.buttons[a],l=g?g.length:0;for(e=0;e<l;e++)f=g[e],f.id==b&&"act"==f.status&&(f[d]&&(h=document.getElementById(f.id))&&(h["image"==f.type?"src":"className"]=f[d]),"sel"==d&&(this.buttons_sel[b]=a))};this.set_pagetitle=function(a){a&&document.title&&(document.title=a)};this.display_message=function(a,b,d){if(this.is_framed())return parent.rcmail.display_message(a,
b,d);if(!this.gui_objects.message)return"loading"!=b&&(this.pending_message=[a,b,d]),1;b=b?b:"notice";var e=this.html_identifier(a),f=b+(new Date).getTime();d||(d=this.message_time*("error"==b||"warning"==b?2:1));"loading"==b&&(e="loading",d=1E3*this.env.request_timeout,a||(a=this.get_label("loading")));if(this.messages[e])return this.messages[e].obj&&this.messages[e].obj.html(a),"loading"==b&&this.messages[e].labels.push({id:f,msg:a}),this.messages[e].elements.push(f),setTimeout(function(){g.hide_message(f,
"loading"==b)},d),f;var h=$("<div>").addClass(b).html(a).data("key",e);$(this.gui_objects.message).append(h).show();this.messages[e]={obj:h,elements:[f]};"loading"==b?this.messages[e].labels=[{id:f,msg:a}]:h.click(function(){return g.hide_message(h)}).attr("role","alert");this.triggerEvent("message",{message:a,type:b,timeout:d,object:h});0<d&&setTimeout(function(){g.hide_message(f,"loading"!=b)},d);return f};this.hide_message=function(a,b){if(this.is_framed())return parent.rcmail.hide_message(a,b);
if(this.gui_objects.message){var d,e,f,h,g=this.messages;if("object"===typeof a)h=$(a),d=h.data("key"),this.hide_message_object(h,b),g[d]&&delete g[d];else for(d in g)for(e in g[d].elements)if(g[d]&&g[d].elements[e]==a)if(g[d].elements.splice(e,1),!g[d].elements.length)this.hide_message_object(g[d].obj,b),delete g[d];else if("loading"==d)for(f in g[d].labels)g[d].labels[f].id==a?delete g[d].labels[f]:(h=g[d].labels[f].msg,g[d].obj.html(h))}};this.hide_message_object=function(a,b){b?a.fadeOut(600,
function(){$(this).remove()}):a.hide().remove()};this.clear_messages=function(){if(this.is_framed())return parent.rcmail.clear_messages();var a,b,d=this.messages;for(a in d)for(b in d[a].elements)d[a].obj&&this.hide_message_object(d[a].obj);this.messages={}};this.show_popup_dialog=function(a,b,d,e){if(this.is_framed())return parent.rcmail.show_popup_dialog(a,b,d,e);var f=$('<div class="popup">');"object"==typeof a?f.append(a):f.html(a);f.dialog($.extend({title:b,buttons:d,modal:!0,resizable:!0,width:500,
close:function(a,b){$(this).remove()}},e||{}));b=$(window);a=b.width();b=b.height();e=f.width();var h=f.height();f.dialog("option",{height:Math.min(b-40,h+75+(d?50:0)),width:Math.min(a-20,e+36)});return f};this.set_page_buttons=function(){this.enable_command("nextpage","lastpage",this.env.pagecount>this.env.current_page);this.enable_command("previouspage","firstpage",1<this.env.current_page)};this.select_folder=function(a,b,d){this.savedsearchlist&&this.savedsearchlist.select("");this.treelist?this.treelist.select(a):
...
d=g.env.mailboxes[this],h=d.id,q=$(k.cloneNode(!1)),v=$("<li>");d.virtual?q.addClass("virtual").attr("aria-disabled","true").attr("tabindex","-1"):q.addClass("active").data("id",d.id);for(d["class"]&&q.addClass(d["class"]);0<=(b=h.indexOf(f,b));)a++,b++;q.css("padding-left",a?16*a+"px":0);q.append($("<span>").text(d.name));v.append(q);e.push(v)});h.append(e).appendTo(d);d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();10<e.length&&d.css("max-height",10*$("li",d)[0].offsetHeight+9);
d.on("click","a.active",function(a){d.data("callback")($(this).data("id"));return!1});this.folder_selector_element=d}d.data("callback",b);this.show_menu("folder-selector",!0,a)};this.show_menu=function(a,b,d){var e="object"==typeof a?a.menu:a,f=$("#"+e),h=d&&d.target?$(d.target):$(f.attr("rel")||"#"+e+"link"),g=rcube_event.is_keyboard(d),l=f.attr("data-align")||"",n=!1;"A"!=h.get(0).tagName&&h.closest("a").length&&(h=h.closest("a"));"string"==typeof a&&(a={menu:e});f.length||(f=this.triggerEvent("menu-get",
{name:e,props:a,originalEvent:d}));if(!f||!f.length)return this.triggerEvent(!1===b?"menu-close":"menu-open",{name:e,props:a,originalEvent:d});f.appendTo(document.body);"undefined"==typeof b&&(b=f.is(":visible")?!1:!0);if(b&&h.length){var m=$(window),p=h.offset(),q=0<=l.indexOf("bottom"),n="menuitem"==h.attr("role")||0<h.closest("[role=menuitem]").length;h.offsetWidth=h.outerWidth();h.offsetHeight=h.outerHeight();!q&&p.top+h.offsetHeight+f.height()>m.height()&&(q=!0);0<=l.indexOf("right")?p.left=
p.left+h.outerWidth()-f.width():n&&(p.left=p.left+h.offsetWidth-5,p.top-=h.offsetHeight);p.left+f.width()>m.width()&&(p.left=m.width()-f.width()-12);p.top=Math.max(0,p.top+(q?-f.height():h.offsetHeight));f.css({left:p.left+"px",top:p.top+"px"})}if(b){for(l=this.menu_stack.length-1;n&&0<=l;l--)$(h).parents("#"+this.menu_stack[l]).length||this.hide_menu(this.menu_stack[l],d);n&&this.menu_stack.length?(f.data("parent",$.last(this.menu_stack)),f.css("z-index",($("#"+$.last(this.menu_stack)).css("z-index")||
0)+1)):!n&&this.menu_stack.length&&this.hide_menu(this.menu_stack[0],d);f.show().attr("aria-hidden","false").data("opener",h.attr("aria-expanded","true").get(0));this.triggerEvent("menu-open",{name:e,obj:f,props:a,originalEvent:d});this.menu_stack.push(e);if(this.menu_keyboard_active=b&&g)this.focused_menu=e,f.find("a,input:not(:disabled)").not("[aria-disabled=true]").first().focus()}else this.hide_menu(e,d);return b};this.hide_menu=function(a,b){if(this.menu_stack.length){for(var d,e=rcube_event.is_keyboard(b),
...
!0;this.set_busy(!1,"",a.lock)}};this.async_upload_form=function(a,b,d){var e=(new Date).getTime(),f="rcmupload"+e,h=this.async_upload_form_frame(f);if(this.env.upload_progress_name){var g=this.env.upload_progress_name,l=$("input[name="+g+"]",a);l.length||(l=$("<input>").attr({type:"hidden",name:g}),l.prependTo(a));l.val(e)}h.bind("load",{ts:e},d);$(a).attr({target:f,action:this.url(b,{_id:this.env.compose_id||"",_uploadid:e,_from:this.env.action}),method:"POST"}).attr(a.encoding?"encoding":"enctype",
...
!1);var b=a.target.files||a.dataTransfer.files,d=window.FormData?new FormData:null,e=(this.env.filedrop.fieldname||"_file")+(this.env.filedrop.single?"":"[]"),f="------multipartformboundary"+(new Date).getTime(),h="--"+f+"\r\n";if(b&&b.length)for(var k=function(){var a=1<b.length,e=(new Date).getTime(),a="<span>"+(a?g.get_label("uploadingmany"):b[0].name)+"</span>";g.add2attachment_list(e,{name:"",html:a,classname:"uploading",complete:!1})||(g.file_upload_id=g.set_busy(!0,"uploading"));h+="--"+f+
...
l=this.env.filedrop.single?0:b.length-1,n=a=0,m;a<=l&&(m=b[n]);n++)if(m.name||(m.name=m.fileName),m.size||(m.size=m.fileSize),m.type||(m.type="application/octet-stream"),!d&&/[^\x20-\x7E]/.test(m.name)&&(m.name_bin=unescape(encodeURIComponent(m.name))),!this.env.filedrop.filter||m.type.match(RegExp(this.env.filedrop.filter))){if(d){if(d.append(e,m),a==l)return k()}else if(window.FileReader){var p=new FileReader;p.onload=function(a,b){return function(d){h+='Content-Disposition: form-data; name="'+
e+'"';h+='; filename="'+(m.name_bin||a.name)+'"\r\n';h+="Content-Length: "+a.size+"\r\n";h+="Content-Type: "+a.type+"\r\n\r\n";h+=p.result+"\r\n";h+="--"+f+"\r\n";if(b==l)return k()}}(m,a);p.readAsBinaryString(m)}else if(m.getAsBinary&&(h+='Content-Disposition: form-data; name="'+e+'"',h+='; filename="'+(m.name_bin||m.name)+'"\r\n',h+="Content-Length: "+m.size+"\r\n",h+="Content-Type: "+m.type+"\r\n\r\n",h+=m.getAsBinary()+"\r\n",h+="--"+f+"\r\n",a==l))return k();a++}};this.start_keepalive=function(){!this.env.session_lifetime||
...
$(b).parent().find(".mailtoprotohandler-status").html(d):$(b).click(function(){g.register_protocol_handler(a);return!1}):$(b).addClass("disabled").click(function(){return!1})};this.browser_capabilities_check=function(){this.env.browser_capabilities||(this.env.browser_capabilities={});void 0===this.env.browser_capabilities.pdf&&(this.env.browser_capabilities.pdf=this.pdf_support_check());void 0===this.env.browser_capabilities.flash&&(this.env.browser_capabilities.flash=this.flash_support_check());
void 0===this.env.browser_capabilities.tif&&this.tif_support_check()};this.browser_capabilities=function(){if(!this.env.browser_capabilities)return"";var a,b=[];for(a in this.env.browser_capabilities)b.push(a+"="+this.env.browser_capabilities[a]);return b.join()};this.tif_support_check=function(){var a=new Image;a.onload=function(){g.env.browser_capabilities.tif=1};a.onerror=function(){g.env.browser_capabilities.tif=0};a.src="program/resources/blank.tif"};this.pdf_support_check=function(){var a=navigator.mimeTypes?
navigator.mimeTypes["application/pdf"]:{},b=navigator.plugins,d=b.length,e=/Adobe Reader|PDF|Acrobat/i;if(a&&a.enabledPlugin)return 1;if(window.ActiveXObject){try{if(new ActiveXObject("AcroPDF.PDF"))return 1}catch(f){}try{if(new ActiveXObject("PDF.PdfCtrl"))return 1}catch(g){}}for(i=0;i<d;i++)if(a=b[i],"String"===typeof a){if(e.test(a))return 1}else if(a.name&&e.test(a.name))return 1;return 0};this.flash_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:
{};if(a&&a.enabledPlugin)return 1;if(window.ActiveXObject)try{if(new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))return 1}catch(b){}return 0};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e;try{e=localStorage.getItem(this.get_local_storage_prefix()+
a)}catch(f){}return null!==e?JSON.parse(e):b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}}}rcube_webmail.long_subject_title=function(g,u){if(!g.title){var a=$(g);a.width()+15*(u||0)>a.parent().width()&&(g.title=a.text())}};
rcube_webmail.long_subject_title_ex=function(g){if(!g.title){var u=$(g),a=$.trim(u.text()),b=$("<span>").text(a).css({position:"absolute","float":"left",visibility:"hidden","font-size":u.css("font-size"),"font-weight":u.css("font-weight")}).appendTo($("body")),d=b.width();b.remove();d+15*$("span.branch",u).width()>u.width()&&(g.title=a)}};rcube_webmail.prototype.get_cookie=getCookie;rcube_webmail.prototype.addEventListener=rcube_event_engine.prototype.addEventListener;
