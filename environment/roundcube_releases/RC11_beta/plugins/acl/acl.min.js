...
rcube_webmail.prototype.acl_save=function(){var a=$("#acluser",this.acl_form).val(),b="",c;$(this.env.acl_advanced?"#advancedrights :checkbox":"#simplerights :checkbox",this.acl_form).map(function(){this.checked&&(b+=this.value)});(c=$("input:checked[name=usertype]",this.acl_form).val())&&"user"!=c&&(a=c);a?b?this.http_request("settings/plugin.acl","_act=save&_user="+urlencode(a)+"&_acl="+b+"&_mbox="+urlencode(this.env.mailbox)+(this.acl_id?"&_old="+this.acl_id:""),this.set_busy(!0,"acl.saving")):
...
a?this.gettext("acl.editperms"):this.gettext("acl.newuser"),d,{modal:!0,closeOnEscape:!0,close:function(a,b){(l.is_framed()?parent.rcmail:l).ksearch_hide();l.acl_form.appendTo(h).hide();$(this).remove();window.focus()}});this.acl_form.appendTo(this.acl_popup).show();"user"==f?g.focus():$("input:checked",n).focus()};rcube_webmail.prototype.acl_class=function(a,b){var c,e,d=0;a=String(a);b=String(b);c=0;for(e=b.length;c<e;c++)-1<a.indexOf(b[c])&&d++;return d==e?"enabled":d?"partial":"disabled"};
