...
a;rcmail.command("menu-open",{menu:"attachmentmenu",id:b})},menu_open:function(a){a&&a.props&&"attachmentmenu"==a.props.menu?this.show_popup("attachmentmenu"):this.open_listmenu()},menu_save:function(a){this.save_listmenu()},body_mouseup:function(a,b){var c,d=rcube_event.get_target(a);for(c in this.popups)!this.popups[c].obj.is(":visible")||d==rcube_find_object(c+"link")||this.popups[c].toggle||d==this.popups[c].obj.get(0)||this.popups[c].editable&&this.target_overlaps(d,this.popups[c].id)||this.popups[c].sticky&&
rcube_mouse_is_over(a,rcube_find_object(this.popups[c].id))||$(d).is(".folder-selector-link")||$(d).children(".folder-selector-link").length||window.setTimeout('rcmail_ui.show_popup("'+c+'",false);',50)},target_overlaps:function(a,b){for(var c=rcube_find_object(b);a.parentNode;){if(a.parentNode==c)return!0;a=a.parentNode}return!1},body_keydown:function(a,b){if(27==rcube_event.get_keycode(a))for(var c in this.popups)this.popups[c].obj.is(":visible")&&this.show_popup(c,!1)},switch_preview_pane:function(a){var b,
c=$("#mailpreviewframe");a.checked?(rcmail.env.contentframe="messagecontframe",mailviewsplit.layer?(mailviewsplit.resize(),mailviewsplit.layer.elm.style.display=""):mailviewsplit.init(),bw.opera&&$("#messagelistcontainer").css({height:""}),c.show(),(b=rcmail.message_list.get_single_selection())&&rcmail.show_message(b,!1,!0)):(c.hide(),bw.ie6||bw.ie7?(b=document.getElementById("mailcontframe"),b.style.bottom=0,b.style.height=parseInt(b.parentNode.offsetHeight)+"px"):($("#mailcontframe").css({height:"auto",
bottom:0}),bw.opera&&$("#messagelistcontainer").css({height:"auto"})),mailviewsplit.layer&&(mailviewsplit.layer.elm.style.display="none"),rcmail.env.contentframe=null,rcmail.show_contentframe(!1));rcmail.command("save-pref",{name:"preview_pane",value:a.checked?1:0})},init_compose_form:function(){var a,b,c,d=["cc","bcc","replyto","followupto"],f=document.getElementById("compose-div"),e=document.getElementById("compose-headers-div");for(a=0;a<d.length;a++)b=d[a],c=$("#_"+b),c.length&&(c.on("change",
{v:b},function(a){this.value&&rcmail_ui.show_header_form(a.data.v)}),""!=c.val()&&rcmail_ui.show_header_form(b));bw.ie&&(rcube_find_object("form").onkeydown=function(a){27==rcube_event.get_keycode(a)&&rcube_event.cancel(a)});$(window).resize(function(){rcmail_ui.resize_compose_body()});$("#compose-container").resize(function(){rcmail_ui.resize_compose_body()});f.style.top=parseInt(e.offsetHeight,10)+3+"px";$(window).resize();$("#contacts-table").css("top",$("#directorylist").height()+24+"px");$("#quicksearchbox").keydown(function(a){13==
