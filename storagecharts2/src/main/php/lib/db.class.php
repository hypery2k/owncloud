...
class OC_DLStCharts
{
     ...
     * 
     * @param $path path
     *            to the folder you want to calculate the total size
     */
    ...
    public static function getPieFreeUsedSpaceRatio()
    {
        if (OC_Group::inGroup(OCP\User::getUser(), 'admin')) {
            $query = OCP\DB::prepare("SELECT stc_id, stc_dayts, oc_uid FROM (SELECT * FROM *PREFIX*storagecharts2 ORDER BY oc_uid,stc_dayts DESC) last GROUP BY oc_uid");
